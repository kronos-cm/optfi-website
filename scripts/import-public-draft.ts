...
interface MethodValidationCard {
  title: string
  state: string
  detail: string
  variant: string
...
interface PublicReportingFeed {
  updatedAt: string
  methodValidationCard: MethodValidationCard
  transparencyUpdates: TransparencyUpdate[]
}

function usage(): void {
  console.error('Usage: tsx scripts/import-public-draft.ts <draft-json-path> [--publish-at ISO8601]')
}
...
const draft = JSON.parse(fs.readFileSync(draftPath, 'utf8')) as DraftInput
const feed = JSON.parse(fs.readFileSync(feedPath, 'utf8')) as PublicReportingFeed
...
const entry: TransparencyUpdate = {
  id,
